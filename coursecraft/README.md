...
**课构**是一款完全在浏览器本地运行的大学课程规划器。它把教务系统导出的教学班信息和培养方案中的推荐学期分开处理：课程包负责提供真实教师、时间和地点，培养方案映射只负责提供“哪门课建议在哪个学期修、属于必修还是选修”。
...
1. 回到 CourseCraft，点击课程仓库中的 **导入课程包**。
...
示例由特定版本培养方案整理，使用前应按照自己入学年份和学校最新方案核对。CourseCraft 的初始页面不预装任何培养方案或个人课程数据。
...
让大模型只输出 JSON，保存为 `.json` 后，在 CourseCraft 中点击 **导入培养方案映射**。学期标识如下：
...
- 课程包：篡改猴导出的 JSON、CSV，以及 CourseCraft/i-home 可识别的 XLSX。
